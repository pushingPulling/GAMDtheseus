...
def cus_col(batches):
    return {'pos': [batch['pos'] for batch in batches],'forces': [batch['forces'] for batch in batches]}
...
class ParticleNetLightning(pl.LightningModule):
    ...
    def get_edge_idx(self, nbrs, pos_jax, mask):
        ...
        center_idx_tsr = torch.as_tensor(center_idx_, device='cpu')
        ...
        neigh_idx_tsr = torch.as_tensor(neigh_idx, device='cpu')
    ...
    def search_for_neighbor(self, pos, nbr_searcher, masking_fn, type_name):
        pos_jax = jax.device_put(pos, jax.devices("cpu")[0])
    ...
    def training_step(self, batch, batch_nb):
        ...
        for b in range(len(gt_lst)):
            ...
            gt = self.scale_force(gt, self.train_data_scaler)
            pos_lst[b] = torch.from_numpy(pos).float()
            gt_lst[b] = gt
    ...
    def val_dataloader(self):
        ...
        return DataLoader(dataset, num_workers=0, batch_size=16, shuffle=False,
                          collate_fn=cus_col)
